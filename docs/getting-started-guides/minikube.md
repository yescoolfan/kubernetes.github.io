...
Minikube is a tool that makes it easy to run Kubernetes locally. Minikube runs a single-node Kubernetes cluster inside a VM on your laptop for users looking to try out Kubernetes or develop with it day-to-day.Then test our private content for inputting here.
...
* Minikube supports Kubernetes features such aswww:
  * DNS
  * NodePorts
